...
public class RestService {
    ...
    public static JSONArray getAllLoggedInUsers() {
        // Perform a get rest call and transform the response to a json array to get all logged in users
        // Return the json array if success, otherwise return nan empty json array
        HttpResponse<JsonNode> answerJson = Unirest.get(USER_URL).asJson();
        if (answerJson.isSuccess()) {
            return answerJson.getBody().getArray();
        } else {
            return new JSONArray();
